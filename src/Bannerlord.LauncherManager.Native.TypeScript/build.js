...
/**
 * Build script for @butr/vortexextensionnative
...
async function main() {
  try {
    ...
    if (["build", "test", "build-napi", "build-ts"].includes(type)) {
    ...
    if (["build", "test", "build-native"].includes(type)) {
      console.log(`Building Bannerlord.LauncherManager.Native (${configuration})`);

      // Verify source directory exists
      const nativeDir = path.resolve("../Bannerlord.LauncherManager.Native");
      if (!fs.existsSync(nativeDir)) {
        throw new Error(
          `Bannerlord.LauncherManager.Native directory not found at: ${nativeDir}`,
        );
      }
    ...
    if (type === "test") {
      ...
      copyItem(`build/${configuration}/launchermanager.node`, "dist/build/launchermanager.node");
