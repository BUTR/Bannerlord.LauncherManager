    ...
    static return_value_void *setGameParameters(param_ptr *p_owner, param_string *p_executable, param_json *p_game_parameters, param_ptr *p_callback_handler, void (*p_callback)(param_ptr *, return_value_void *)) noexcept
    {
        ...
        try
        {
            ...
            if (std::this_thread::get_id() == manager->MainThreadId)
            {
                ...
                manager->FSetGameParameters.Call({executable, gameParameters});
            ...
            else
            {
                ...
                const auto callback = [functionName, manager, p_executable, p_game_parameters, &result, &mtx, &cv, &completed](Napi::Env env, Napi::Function jsCallback)
                {
                    ...
                    try
                    {
                        ...
                        jsCallback.Call({executable, gameParameters});
                ...
                const auto status = manager->TSFNSetGameParameters.BlockingCall(callback);
                if (status != napi_ok)
                {
    ...
    static return_value_void *sendNotification(param_ptr *p_owner, param_string *p_id, param_string *p_type, param_string *p_message, param_uint displayMs, param_ptr *p_callback_handler, void (*p_callback)(param_ptr *, return_value_void *)) noexcept
    {
        ...
        try
        {
            ...
            if (std::this_thread::get_id() == manager->MainThreadId)
            {
                ...
                manager->FSendNotification.Call({id, type, message, displayMs_});
            ...
            else
            {
                ...
                const auto callback = [functionName, manager, p_id, p_type, p_message, displayMs, &result, &mtx, &cv, &completed](Napi::Env env, Napi::Function jsCallback)
                {
                    LoggerScope callbackLogger(NAMEOFWITHCALLBACK(functionName, callback));
                    try
                    {
                        ...
                        jsCallback.Call({id, type, message, displayMs_});
                ...
                const auto status = manager->TSFNSendNotification.BlockingCall(callback);
                if (status != napi_ok)
                {
                    logger.Log("BlockingCall failed with status: " + std::to_string(status));
                    return Create(return_value_void{Copy(u"Failed to queue async call")});
    ...
    static return_value_void *sendDialog(param_ptr *p_owner, param_string *p_type, param_string *p_title, param_string *p_message, param_json *p_filters, param_ptr *p_callback_handler, void (*p_callback)(param_ptr *, return_value_string *)) noexcept
    {
        ...
        try
        {
            ...
            if (std::this_thread::get_id() == manager->MainThreadId)
            {
                ...
                const auto jsResult = manager->FSendDialog.Call({type, title, message, filters});
            ...
            else
            {
                ...
                const auto callback = [functionName, manager, p_type, p_title, p_message, p_filters, p_callback_handler, p_callback, &result, &mtx, &cv, &completed](Napi::Env env, Napi::Function jsCallback)
                {
                    LoggerScope callbackLogger(NAMEOFWITHCALLBACK(functionName, callback));
                    try
                    {
                        ...
                        const auto jsResult = jsCallback.Call({type, title, message, filters});
                ...
                const auto status = manager->TSFNSendDialog.BlockingCall(callback);
                if (status != napi_ok)
                {
                    logger.Log("BlockingCall failed with status: " + std::to_string(status));
                    return Create(return_value_void{Copy(u"Failed to queue async call")});
    ...
    static return_value_void *writeFileContent(param_ptr *p_owner, param_string *p_file_path, param_data *p_data, param_int length, param_ptr *p_callback_handler, void (*p_callback)(param_ptr *, return_value_void *)) noexcept
    {
        ...
        try
        {
            ...
            if (std::this_thread::get_id() == manager->MainThreadId)
            {
                ...
                manager->FWriteFileContent.Call({filePath, data});
    ...
    static return_value_void *setModuleViewModels(param_ptr *p_owner, param_json *p_module_view_models, param_ptr *p_callback_handler, void (*p_callback)(param_ptr *, return_value_void *)) noexcept
    {
        ...
        try
        {
            ...
            if (std::this_thread::get_id() == manager->MainThreadId)
            {
                ...
                manager->FSetModuleViewModels.Call({moduleViewModels});
            ...
            else
            {
                ...
                const auto callback = [functionName, manager, p_module_view_models, &result, &mtx, &cv, &completed](Napi::Env env, Napi::Function jsCallback)
                {
                    LoggerScope callbackLogger(NAMEOFWITHCALLBACK(functionName, callback));
                    try
                    {
                        ...
                        jsCallback.Call({moduleViewModels});
                ...
                const auto status = manager->TSFNSetModuleViewModels.BlockingCall(callback);
                if (status != napi_ok)
                {
                    logger.Log("BlockingCall failed with status: " + std::to_string(status));
                    return Create(return_value_void{Copy(u"Failed to queue async call")});
