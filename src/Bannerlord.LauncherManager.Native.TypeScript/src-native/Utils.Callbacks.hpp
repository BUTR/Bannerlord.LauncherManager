...
template <typename T> struct ResultCallbackTraits;

template <> struct ResultCallbackTraits<return_value_void>
{
...
template <> struct ResultCallbackTraits<return_value_json>
{
...
template <> struct ResultCallbackTraits<return_value_string>
{
...
template <> struct ResultCallbackTraits<return_value_bool>
{
...
template <typename T> T *CreateErrorResult(const std::u16string &errorMsg);

template <> inline return_value_string *CreateErrorResult(const std::u16string &errorMsg)
{
    return Create(return_value_string{Copy(errorMsg), nullptr});
}

template <> inline return_value_json *CreateErrorResult(const std::u16string &errorMsg)
{
    return Create(return_value_json{Copy(errorMsg), nullptr});
}

template <> inline return_value_data *CreateErrorResult(const std::u16string &errorMsg)
{
    return Create(return_value_data{Copy(errorMsg), nullptr, 0});
}

template <> inline return_value_void *CreateErrorResult(const std::u16string &errorMsg)
{
...
inline void HandlePromiseOrValue(const Napi::Env &env, const Napi::Value &jsResult, param_ptr *p_callback_handler, void (*p_callback)(param_ptr *, TReturnValue *),
                                 TConverter converter, std::mutex *mtx = nullptr, std::condition_variable *cv = nullptr, bool *completed = nullptr,
                                 return_value_void **result = nullptr)
{
    ...
    const bool isPromiseLike = jsResult.IsObject() && jsResult.As<Napi::Object>().Has("then") && jsResult.As<Napi::Object>().Get("then").IsFunction();
...
// Convenience wrappers for backward compatibility
inline void HandleStringPromiseOrValue(const Napi::Env &env, const Napi::Value &jsResult, param_ptr *p_callback_handler, void (*p_callback)(param_ptr *, return_value_string *),
                                       std::mutex *mtx = nullptr, std::condition_variable *cv = nullptr, bool *completed = nullptr, return_value_void **result = nullptr)
{
    HandlePromiseOrValue(env, jsResult, p_callback_handler, p_callback, ConvertToStringResult, mtx, cv, completed, result);
}

inline void HandleJsonPromiseOrValue(const Napi::Env &env, const Napi::Value &jsResult, param_ptr *p_callback_handler, void (*p_callback)(param_ptr *, return_value_json *),
                                     std::mutex *mtx = nullptr, std::condition_variable *cv = nullptr, bool *completed = nullptr, return_value_void **result = nullptr)
{
    HandlePromiseOrValue(env, jsResult, p_callback_handler, p_callback, ConvertToJsonResult, mtx, cv, completed, result);
}

inline void HandleDataPromiseOrValue(const Napi::Env &env, const Napi::Value &jsResult, param_ptr *p_callback_handler, void (*p_callback)(param_ptr *, return_value_data *),
                                     std::mutex *mtx = nullptr, std::condition_variable *cv = nullptr, bool *completed = nullptr, return_value_void **result = nullptr)
{
    HandlePromiseOrValue(env, jsResult, p_callback_handler, p_callback, ConvertToDataResult, mtx, cv, completed, result);
}

inline void HandleVoidPromiseOrValue(const Napi::Env &env, const Napi::Value &jsResult, param_ptr *p_callback_handler, void (*p_callback)(param_ptr *, return_value_void *),
                                     std::mutex *mtx = nullptr, std::condition_variable *cv = nullptr, bool *completed = nullptr, return_value_void **result = nullptr)
{
    HandlePromiseOrValue(
        env, jsResult, p_callback_handler, p_callback, [](const Napi::Value &) -> return_value_void * { return Create(return_value_void{nullptr}); }, mtx, cv, completed, result);
}

// Helper for outer exception handling in callbacks - returns error result
template <typename TResult, typename Func> inline TResult *CallbackWithExceptionHandling(LoggerScope &logger, Func &&func) noexcept
{
    try
    {
        return func();
